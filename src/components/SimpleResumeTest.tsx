...
const SimpleResumeTest = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send request
      console.log('Sending direct request to backend...');
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Simple Resume Test</h1>
      
      <form onSubmit={handleSubmit} className="space-y-4 mb-8 p-4 border rounded">
        <div>
              ...
              {results.results.map((result: any, index: number) => (
                <div key={index} className="p-4 border rounded bg-white">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-lg">{Math.round(result.cosine_similarity * 100)}% Match</span>
                  </div>
...
export default SimpleResumeTest;
